...
export async function getPlayerStyleResponse(query: QuerySource): Promise<unknown> {
  const nickname = getQueryValue(query, "nickname");
  if (typeof nickname !== "string" || nickname.trim() === "") {
    throw new ApiError(400, "bad_input", "nickname is required");
  }

  const sameName = parseSameName(getQueryValue(query, "sameName"));
  const count = parseCount(getQueryValue(query, "count"));
  const explicitPlayerId = parsePositiveSafeInteger(getQueryValue(query, "playerId"), "playerId");
  const explicitLatestTimestamp = parsePositiveSafeInteger(
    getQueryValue(query, "latestTimestamp"),
    "latestTimestamp"
  );
